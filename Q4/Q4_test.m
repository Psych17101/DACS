...
mean_E2 = 8.44e9; std_E2 = 0.2e9;
mean_G12 = 7e9; std_G12 = 0.1e9;
mean_v12 = 0.2; std_v12 = 0.001;
X_T = 1500.e6; % Fiber
X_C = 1200.e6; %
Y_T = 50.e6; % Matrix
Y_C = 250.e6;
...
N = [100000, 400000]; % resultant force in N/m
...
for i = 1:length(N) % iterate between the two forces
    F = [N(i)*cosd(theta(i)); N(i)*sind(theta(i)); 0]; % load vector N/m
    ply_failure = zeros(num_simulations, 1); % array to store ply failure results
    
    for j = 1:num_simulations %100 times
        % generate random samples for material properties
        E1_samples = normrnd(mean_E1, std_E1, 1, num_samples); % Pa
        E2_samples = normrnd(mean_E2, std_E2, 1, num_samples); % Pa
        G12_samples = normrnd(mean_G12, std_G12, 1, num_samples); % Pa
        ...
        for k = 1:num_samples %30 times
            ...
            for l = 1:Nplies
                ...
                stress_samples(:,k) = Q*strain_samples(:,k); % global sigmaxx % Pa
                ...
                fe(j,k,l) = puckfailure(sigma_loc,E1_samples(k),E_x(k),v12_samples(k),X_T,X_C);  % (num_samples,layer)
                ...
                if fe(j,k,l) > max_fe(j,k)
                    max_fe(j,k) = fe(j,k,l); % failure index under puck criterion
                    numberply(j,k,l) = l;
                end
            end   
        end
